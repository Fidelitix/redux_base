...
#    /usr/bin/ssh giveme@giveme.adgiear.com events trader $FARM $TYPE json $EXPORTED_HOUR >> $LOG_FILE
    /usr/bin/ssh giveme@giveme.adgear.com events trader $FARM $TYPE avro $EXPORTED_HOUR >> $LOG_FILE
#    gsutil cp $LOG_FILE gs://adgear-raw/$(basename $LOG_FILE)
#    bq ls Adgear_delivery_logs | grep $(sed 's/-//g' <(echo $CURRENT_DATE)) >/dev/null
#    if $(test $hour -eq 0 -a $? -eq 0); then
#        echo The table already exists. This means we are in a bad state.
#        echo Delete then run me again
#        exit
#    fi
#    bq load --source_format=AVRO Adgear_delivery_logs.$(sed 's/-//g' <(echo $CURRENT_DATE)) gs://adgear-raw/$(basename $LOG_FILE)
  done
